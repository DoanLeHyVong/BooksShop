...
public class MyPageController {
	...
	public String myPgae(HttpSession session,HttpServletRequest request,Model model) {
		...
		if (list.size()!=0) {
			...
			for (Item i : list) {
				...
				OderLine oderline = new OderLine(oid, bid, quantity,i.getPrice()*i.getQuantity());
	...
	@GetMapping(value = "/myOrderPending")
	...
	public String buyBook(Model model,HttpSession session,HttpServletRequest request) {
		DAO dao = new DAO();
		Account ac = (Account) session.getAttribute("account");
		int uid = ac.getId();
		
		String id = request.getParameter("bookid");
		String quantityBook = request.getParameter("quantity");
		int num=1,bid =0;
		try {
			 bid = Integer.parseInt(id);
			 num = Integer.parseInt(quantityBook);
			 LocalDate localDate = LocalDate.now();
		     String day = String.valueOf(localDate);
		     Book book = dao.getBookById(bid);
			 Order order = new Order(0,uid, num, day,book.getPrice()*num, 0);		 
			 dao.insertOrder(order);
			 
			 int oid = dao.getLastOrderId();
			 OderLine oderline = new OderLine(oid, bid, num,book.getPrice()*num);
			 dao.insertOrderLine(oderline);
			 dao.updateBookSold(bid, num);
		} catch (Exception e) {
	...
	@GetMapping(value = "/delete/OrderPending")
	public String deleteOrder(HttpServletRequest request,Model model,HttpSession session) {
		...
		return "redirect:/myOrderPending";
	}
